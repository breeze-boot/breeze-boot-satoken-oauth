...
import com.breeze.boot.ai.service.IAiChatLLMService;
...
public class ChatLLMController {
    ...
    private final IAiChatLLMService chatLLMService;

    @Operation(summary = "列表")
    @SaCheckPermission("chat:llm:list")
    @GetMapping
    public Result<List<?>> list() {
        List<AiChatLLM> list = this.chatLLMService.list();
    ...
    @PostMapping
    public Result<?> save(@Valid @ParameterObject @RequestBody AiChatLLM chatLLM) {
    ...
    @PutMapping("/{id}")
    public Result<?> edit(@Parameter(description = "模型ID") @PathVariable Long id, @ParameterObject @Valid @RequestBody AiChatLLM chatLLM) {
